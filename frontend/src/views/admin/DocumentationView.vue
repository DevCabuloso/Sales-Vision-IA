<template>
  <div class="admin-page">
    ...
    <footer class="doc-footer">
      ...
      repositório em 2026-07-12. Rotas, colunas e comportamento podem mudar; trate como referência, não como contrato.
    </footer>
  </div>
</template>
...
const attentionPoints = [
  { level: 'success', title: 'Corrigido: endpoint de debug público removido', desc: '<code>GET /api/diag?key=sdr2025</code> vazava mensagens/leads de <strong>todos os tenants</strong> (sem filtro de tenant_id) pra quem soubesse a chave estática — foi removido em 2026-07-12. Não existia mais nenhum <code>/webhooks/debug-msgs</code> (citado numa versão antiga desta página). O único debug remanescente é <code>GET /api/chat/debug-recent</code>, que já exige sessão autenticada e é escopado por tenant.' },
  { level: 'success', title: 'Corrigido: cookie de sessão agora usa secure em produção', desc: '<code>sdr_token</code> passou a usar <code>secure: config.env === \'production\'</code> (true atrás do HTTPS do nginx, false só em dev local) em vez de <code>secure: false</code> fixo.' },
  { level: 'success', title: 'Corrigido (parcialmente): guard-rail de teste contra leituras sem filtro de tenant', desc: 'Ativar RLS de verdade exigiria o backend parar de usar a <code>service_role</code> key (mudança grande de arquitetura — esse role ignora RLS por definição). Como mitigação prática, foi adicionado <code>tenant-isolation-guard.test.js</code>: um teste que varre todas as rotas e falha se alguma leitura numa tabela com tenant_id não tiver <strong>nenhum</strong> filtro (nem <code>.eq</code>, <code>.in</code>, <code>.match</code>, etc.) — o mesmo padrão que vazou dados no antigo <code>/api/diag</code>. Isso não cobre tudo (leituras filtradas só por uma FK de uma linha já validada em outra query continuam sem garantia automática, exigiriam análise de fluxo de dados), mas pega o caso mais grave. <code>migration_tenant_safety_triggers.sql</code> continua sendo a única proteção de banco, e só cobre escrita cruzada, não leitura.' },
  { level: 'success', title: 'Corrigido: schema do banco versionado de novo no repositório', desc: 'Commit "SQL REMOVIDO" (2026-07-08) tinha apagado os 14 arquivos .sql da árvore de trabalho. Em 2026-07-12, <code>backend/src/db/schema.sql</code> foi reconstruído por introspecção read-only do Supabase (OpenAPI do PostgREST) — cobre tabelas, colunas, tipos, defaults e foreign keys das 33 tabelas atuais. Limitação: não é um pg_dump oficial, então constraints CHECK/UNIQUE e índices não-PK não aparecem; o ideal é substituir por um pg_dump --schema-only real na próxima janela de manutenção.' },
  { level: 'success', title: 'Mitigado: guard-rail contra escalar o backend sem migrar o cache do orquestrador', desc: '<code>services/orchestrator.js</code> mantém caches em memória de processo (TTL de config de tenant, dedupe de mensagens enviadas pela plataforma) que divergiriam se alguém rodasse mais de uma instância PM2. Em vez de só documentar o risco, <code>src/utils/assertSingleInstance.js</code> agora crasha no boot (chamado em <code>server.js</code>) qualquer instância com <code>NODE_APP_INSTANCE</code> ≠ "0" — o PM2 injeta essa env var a partir da 2ª instância. <code>ecosystem.config.cjs</code> ganhou <code>instances: 1</code> explícito com o motivo comentado. Continua sendo uma trava, não uma solução: pra escalar de verdade os caches precisariam ir pra um store compartilhado (ex: Redis).' },
  { level: 'success', title: 'Verificado: mapeamento de ACK da Evolution bate com a especificação pública', desc: 'Os valores numéricos de <code>ACK_STATUS_MAP</code> (0–5) batem exatamente com o enum <code>WAMessageStatus</code> do protobuf da Baileys (ERROR/PENDING/SERVER_ACK/DELIVERY_ACK/READ/PLAYED) — biblioteca que a Evolution API usa por baixo — e os nomes batem com o campo <code>update.status</code> que a Evolution v2 manda no webhook. Reforcei os testes de <code>parseWebhookStatus</code> pra travar essa tabela (12 combinações via <code>it.each</code>) e cobrir os 4 formatos de payload aceitos (<code>update.status</code>, <code>status</code>, <code>update.ack</code>, <code>ack</code>). Ainda não é a mesma coisa que ver um payload real de produção — vale conferir os logs do PM2 (grep por "ack não reconhecido") depois do próximo disparo em massa, mas o risco de mapeamento errado caiu bastante.' },
  { level: 'success', title: 'Corrigido: SSRF crítico no construtor de fluxos (nó webhook)', desc: 'Qualquer usuário autenticado do tenant podia criar um fluxo com um nó webhook apontando pra rede interna (sem checagem de role) e reinjetar a resposta numa mensagem via <code>{{variavel}}</code>, exfiltrando dados internos pro próprio WhatsApp. Criado <code>utils/ssrfGuard.js</code> (bloqueia IP privado/loopback por hostname E resolução DNS, revalida cada hop de redirect) e aplicado no nó webhook do <code>flowEngine.js</code>, em <code>custom-apis.js</code> (que só validava a URL original, não redirects), no <code>media_url</code> de follow-ups (<code>scheduler.js</code>) e forward de mídia (<code>chat.js</code>), e no baseUrl legado da Evolution por tenant (<code>evolution.js</code> + validação em <code>integrations.js</code> no momento de salvar).' },
  { level: 'success', title: 'Corrigido: injeção de filtro no .or() de contacts.js', desc: 'O termo de busca (<code>?search=</code>) ia direto pro filtro <code>.or()</code> do PostgREST sem escapar vírgulas/aspas, permitindo injetar condições extras de filtro. Agora o valor é escapado e envolvido em aspas antes de montar o filtro.' },
  { level: 'success', title: 'Performance: histórico de IA, paginação e cache em pontos sem limite', desc: 'Histórico de mensagens pro contexto da IA (orchestrator.js e nó "ia" do flowEngine) agora busca só as últimas 40 mensagens em vez da conversa inteira. <code>leads.js</code> (<code>/:id/messages</code>, <code>/:id/analyze</code>) ganhou o mesmo tipo de limite. <code>getTenantFlags</code>/<code>getConnectedChannel</code> (chamados a cada mensagem enviada) agora usam um cache TTL compartilhado (<code>utils/ttlCache.js</code>) — evita ~10 mil queries evitáveis por campanha de disparo em massa. <code>contacts.js</code>/<code>reports.js</code>/<code>admin.js</code>/<code>broadcast.js</code> ganharam tetos de segurança em queries que cresciam sem limite. Em <code>orchestrator.js</code>, até 5 updates sequenciais no lead por mensagem recebida viraram 1 update combinado (e um update de <code>channel_id</code> inteiramente redundante — o upsert já gravava o mesmo valor — foi removido).' },
  { level: 'success', title: 'Performance: PDF do dashboard não pesa mais no carregamento inicial', desc: '<code>jspdf</code>/<code>jspdf-autotable</code> (~420KB) saíram de import estático no <code>DashboardView.vue</code> pra dynamic import dentro do handler de exportação — só carrega pra quem realmente clica em "Baixar PDF".' },
  { level: 'success', title: 'Cobertura de testes: 79%→88% statements, 69%→76% branches', desc: 'Lacunas identificadas numa varredura (chat.js 42%→81%, orchestrator.js 58%→82%, flowEngine.js 64%→84%, evolution.js 81%→87%) foram preenchidas: ciclo de vida completo de ticket, upload/forward/localização, group-access, agendamentos e acompanhamentos em chat.js; download de mídia + transcrição de áudio + reanálise de IA em background em orchestrator.js; nós ia/delay/passo-transferir/passo-encerrar + todos os operadores de condição em flowEngine.js; usage.js ganhou teste dedicado (não tinha nenhum). 727 testes passando no backend, 210 no frontend.' },
  { level: 'success', title: 'Corrigido: xlsx atualizado pro build oficial patcheado da SheetJS', desc: '<code>npm audit</code> apontava prototype pollution + ReDoS em alta severidade na versão 0.18.5 (a última publicada no registry do npm — a SheetJS parou de publicar lá e passou a distribuir só pelo próprio CDN). Backend agora instala <code>xlsx</code> direto de <code>cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz</code> (mesma API, nenhuma mudança de código) — <code>npm audit</code> não acusa mais essa vulnerabilidade. No frontend, <code>xlsx</code> nunca era importado em lugar nenhum (o upload de .xlsx/.xls só envia o arquivo bruto pro backend, quem faz o parsing é o backend) — removido do <code>package.json</code> e do chunk manual do <code>vite.config.js</code> como peso morto.' },
]
</script>
